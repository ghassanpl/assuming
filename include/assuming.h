...
#define Assuming(exp, ...) { if (decltype(auto) _assuming_exp_v = (exp); GSL_UNLIKELY(!_assuming_exp_v)) \
	::ghassanpl::ReportAssumptionFailure(ASSUMING_SL, #exp " will evalute to true", { { #exp, fmt::format("{}", _assuming_exp_v) } }, ::ghassanpl::detail::AdditionalDataToString(__VA_ARGS__)); }

#define AssumingNull(exp, ...) { if (decltype(auto) _assuming_exp_v = (exp); GSL_UNLIKELY(_assuming_exp_v != nullptr)) \
	::ghassanpl::ReportAssumptionFailure(ASSUMING_SL, #exp " will be null", { { #exp, fmt::format("{}", fmt::ptr(_assuming_exp_v)) } }, ::ghassanpl::detail::AdditionalDataToString(__VA_ARGS__)); }

#define AssumingNotNull(exp, ...) { if (decltype(auto) _assuming_exp_v = (exp); GSL_UNLIKELY(_assuming_exp_v == nullptr)) \
	::ghassanpl::ReportAssumptionFailure(ASSUMING_SL, #exp " will not be null", { { #exp, fmt::format("{}", fmt::ptr(_assuming_exp_v)) } }, ::ghassanpl::detail::AdditionalDataToString(__VA_ARGS__)); }
...
#define AssumingBinOp(a, b, op, text, ...) { decltype(auto) _assuming_a_v = (a); decltype(auto) _assuming_b_v = (b); if (GSL_UNLIKELY(!(_assuming_a_v op _assuming_b_v))) \
	::ghassanpl::ReportAssumptionFailure(ASSUMING_SL, #a " will " text " " #b, { { #a, fmt::format("{}", _assuming_a_v) }, { #b, fmt::format("{}", _assuming_b_v) } }, ::ghassanpl::detail::AdditionalDataToString(__VA_ARGS__)); }
...
#define AssumingEmpty(exp, ...) { using std::empty; using std::size; if (decltype(auto) _assuming_exp_v = (exp); GSL_UNLIKELY(!empty(_assuming_exp_v))) \
	::ghassanpl::ReportAssumptionFailure(ASSUMING_SL, #exp " will be empty", { { "size of " #exp, fmt::format("{}", size(_assuming_exp_v)) } }, ::ghassanpl::detail::AdditionalDataToString(__VA_ARGS__)); }
...
#define AssumingNotEmpty(exp, ...) { using std::empty; using std::size; if (decltype(auto) _assuming_exp_v = (exp); GSL_UNLIKELY(empty(_assuming_exp_v))) \
	::ghassanpl::ReportAssumptionFailure(ASSUMING_SL, #exp " will not be empty", { { "size of " #exp, fmt::format("{}", size(_assuming_exp_v)) } }, ::ghassanpl::detail::AdditionalDataToString(__VA_ARGS__)); }

#define AssumingNullOrEmpty(exp, ...) { using std::empty; using std::size; if (decltype(auto) _assuming_exp_v = (exp); GSL_UNLIKELY(!::ghassanpl::::IsNullOrEmpty(_assuming_exp_v))) \
...
#define AssumingNotNullOrEmpty(exp, ...) { using std::empty; using std::size; if (decltype(auto) _assuming_exp_v = (exp); GSL_UNLIKELY(::ghassanpl::::IsNullOrEmpty(_assuming_exp_v))) \
...
#define AssumingNullOrEmpty(exp, ...) { using std::empty; using std::size; GSL_ASSUME(::ghassanpl::::IsNullOrEmpty(exp));  }
#define AssumingNotNullOrEmpty(exp, ...) { using std::empty; using std::size; GSL_ASSUME(!::ghassanpl::::IsNullOrEmpty(exp)); }
